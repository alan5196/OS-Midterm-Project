...
int main(int argc, char *argv[])
{
    ...
    fscanf(inputFile, "%d", &numberOfPlayers); // numberOfPlayers - 1 is the end of the array
    
    //t1 = time(NULL);
    pthread_t thread1, thread2;
    pthread_create(&thread1, NULL, (void *)readFunction, inputFile);
    pthread_create(&thread2, NULL, (void *)readFunction, inputFile);
...
void *readFunction(void *param)
{
    while(i < numberOfPlayers){
        ...
        //printf("i is %d :%d", i, playersData[i].id);
        fscanf(param, "%d", &playersData[i].rankValue);
        //printf("%d\n", playersData[i].rankValue);
        i++;
